...
public class RabbitConfig {
	...
	public Queue createGenerativeAIAlarmContentsDlq() {
		return QueueBuilder
			...
			.withArgument("x-dead-letter-routing-key", AI_GENERATIVE_ALARM_CONTENTS_QUEUE_NAME)
			.build();
	}
	...
	public Queue createGeneratedAIAlarmContentsResponseDlq() {
		return QueueBuilder
			...
			.withArgument("x-message-ttl", 60000)
			.withArgument("x-dead-letter-exchange", "")
			.withArgument("x-dead-letter-routing-key", AI_GENERATED_ALARM_CONTENTS_RESPONSE_QUEUE_NAME)
			.build();
	}
